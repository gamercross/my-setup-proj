// 404 / 공통 에러 핸들러
// 오류 응답 봉투는 RFC 9457 스타일(ADR-0017): { type, title, status, detail, errors?, request_id }.
// 스택·SQLite 영문 원문은 절대 응답 본문에 넣지 않는다(로그에만).
...
function notFoundHandler(req, res) {
  sendProblem(res, 'not_found', { detail: '요청한 경로를 찾을 수 없습니다.' });
}
...
function errorHandler(err, req, res, next) {
  try {
    ...
    if (err.type === 'entity.too.large') {
      return sendProblem(res, 'payload_too_large', { detail: '요청 본문이 너무 큽니다.' });
    }
    if (
    ...
    return sendProblem(res, 'internal_error', { detail: '서버 내부 오류가 발생했습니다.' });
  } catch (handlerErr) {
    console.error('에러 핸들러 자체 오류:', handlerErr);
    if (!res.headersSent) {
      sendProblem(res, 'internal_error', { detail: '서버 내부 오류가 발생했습니다.' });
    }
  }
}
