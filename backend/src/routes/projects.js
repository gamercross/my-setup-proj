...
router.get('/', (req, res) => {
  try {
    ...
    sendServiceError(err, res, { logPrefix: '프로젝트 조회 실패:', failMessage: '프로젝트를 불러오지 못했습니다.' });
  }
});
...
router.get('/:id', (req, res) => {
  try {
    ...
    if (!project) {
      return sendProblem(res, 'not_found', { detail: '프로젝트를 찾을 수 없습니다.' });
    }
    res.json({ project });
  } catch (err) {
    sendServiceError(err, res, { logPrefix: '프로젝트 조회 실패:', failMessage: '프로젝트를 불러오지 못했습니다.' });
...
router.post('/', (req, res) => {
  try {
    ...
    sendServiceError(err, res, { logPrefix: '프로젝트 생성 실패:', failMessage: '프로젝트를 생성하지 못했습니다.' });
  }
});
...
router.put('/:id', (req, res) => {
  try {
    ...
    sendServiceError(err, res, { logPrefix: '프로젝트 수정 실패:', failMessage: '프로젝트를 수정하지 못했습니다.' });
  }
});
...
router.delete('/:id', (req, res) => {
  try {
    ...
    sendServiceError(err, res, { logPrefix: '프로젝트 삭제 실패:', failMessage: '프로젝트를 삭제하지 못했습니다.' });
  }
});
