...
**병합 대기 PR 4개(전부 CI green, 순서대로 병합):** #43(✅ 이미 병합) → **#44 → #45 → #46**.
사용자가 자기 전에 "커밋으로만 남겨두라"고 해서 병합은 안 함.
...
| **#44** | P2 캔버스 v2 + **ADR-0032 채택** + PO-13/14 결정 (docs) | **OPEN · CI green · 병합 대기 (1순위)** |
| **#45** | **P4.5 사이드바 셸 구현** (`feat`, 커밋 `1c5ee54`, 34파일) — #44 위에 스택 | **OPEN · CI green · 병합 대기 (2순위)** |
| **#46** | ADR-0028 §결정4 를 PO-7(리스트/보드 토글)에 맞춤 + 이 문서 갱신 (docs) | **OPEN · CI green · 병합 대기 (3순위)** |
...
2. #44→#45→#46 이 병합됐는지 확인. 스택이라 순서대로 병합해야 깔끔(#44 먼저). 충돌 시 대개 PERSONAL_OS 표 인접 행 — 2줄 수동 정리.
3. 병합 완료 후 병합된 로컬 브랜치 삭제.
4. 데모 재배포 확인(`main` 의 `frontend/**` 변경 → 자동). #45 병합 후 데모에 사이드바 등장.
5. **P5 착수**: ADR-0028 채택 여부 사용자에게 확인 → `/feature`.
...
| P4 | T5 공통 컴포넌트 | ✅ 병합됨 |
| **P4.5** | 사이드바 셸 (UI_STYLE v2 / ADR-0032 채택) | ✅ 구현 완료 — **PR #45 병합 대기** |
