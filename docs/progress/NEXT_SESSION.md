...
- **P11** 지식 축적 추세·역량 지도 — 체크인 빈도·OKR 평균 달성률·태그 분포 종합 뷰
  (ADR-0036, `GET /api/knowledge-trend`, PR #74)
- **P12** 레퍼런스 자료 요약 절차 추적 — `reference_materials`+`reference_summary_steps`
  2테이블, 요약 단계 이력(ADR-0037, PR #76) — **§5-3 정합성 감사의 마지막 공백을 닫음
  (공백 1→0)**
...
- §1-2 고객과 니즈, §2-5 니즈→기능 매핑, §4-7 문제 도식화, §6-4 성과 수치·프로젝트를
  마치면 얻는 것 — REVERSE_PLAN.md에 신설(PR #73), 공유 아티팩트(역 계획서·대시보드 OS)와
  story 저장소에도 동기화 완료
- §3-2·3-5 에이전트 파이프라인·GitHub 사용 방식을 다이어그램 중심으로 재작성, story
  사이트에도 flow-row 카드로 반영 (PR #78)
- **콘텐츠 다듬기 완료** — 역 계획서 밀도 높은 표(§2-3 스택, §5 서비스)를 카드 레이아웃으로
  전환, "Cyber Brutalism" 방향(무채색+블루 accent, 굵은 제목, 번호 매긴 섹션 라벨) 적용
  완료. 더 다듬을 표가 보이면 그때그때 추가할 것 — 강제 남은 항목 아님.
...
- `docs/readme-refresh` 브랜치 (PR #79, **의도적으로 병합 안 하고 열어둠**) — 저장소
  전체 README·문서의 수치 드리프트(ADR 개수·테스트 개수·`verify.sh` 통과 수·오래된
  "제안" 표기 등)를 훑어 정정하는 작업. 계속 이어서 쌓이는 중.
...
`docs/readme-refresh`(PR #79) 상태를 먼저 확인해라 — 병합 여부와 상관없이 이 브랜치
위에서 이어서 작업(사용자가 명시적으로 "열어두고 나중에 합쳐서 병합" 요청). 새 문서
드리프트가 더 보이면 같은 브랜치에 커밋 추가.
