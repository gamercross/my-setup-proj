...
| 보안 보완 S4 (OAuth 키 회전 런북) | ✅ 2026-09-16 | REVERSE_PLAN §4-5 구 S4(OAuth 토큰 암호화 키 회전 절차 없음) 해소. 코드 변경 없음 — `docs/setup/ENV_REFERENCE.md` `TOKEN_ENCRYPTION_KEY` 절에 회전·유출 대응 런북 추가(정기 주기 없음, 유출 트리거 시 회전 + Google 계정 OAuth 권한 취소 절차 포함). `ADR-0024`·`RISKS.md` R-14(🔴→🟢) 갱신, §4-5 남은 약점 7개→6개(S3·S5는 별도 PR #81), §4-7 도식 S4 노드 삭제. 검증: `bash scripts/check-docs.sh` 11/0/0(문서만 변경) |
| 다음 | ⏳ | **앱 통합** (PERSONAL_OS §4-3 — 통합 실행 스크립트 `bash scripts/dev.sh` ✅ ADR-0016 1항 채택 2026-09-09; 데모↔실서버 경로 패리티 ✅; 루프백 바인딩 ✅ 2026-09-15; 남은 것 ADR-0016 2~4항 `APP_ENV` 주입 주체 포함) + P7·P8·P9·P10·P12 로컬 수동 검증 백로그(TC-ACT-M/TC-AGENT-M, TC-OKR-M/TC-PLAN-M, TC-P9-M, TC-DEV-M1~3) |
