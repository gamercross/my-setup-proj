...
flowchart LR
  ...
  subgraph BUILD["③ 빌드 (/feature 파이프라인)"]
    ...
    P11["P11 · 지식 축적 추세·역량 지도<br/>GET /api/knowledge-trend (파생 뷰)<br/>+ 지식 지도 위젯"]
    P12["P12 · 레퍼런스 자료 요약 절차 추적<br/>reference_materials + reference_summary_steps<br/>+ 레퍼런스 위젯 (ADR-0037)"]
  end
  ...
  style P11 fill:#dcfce7,stroke:#16a34a
  style P12 fill:#dcfce7,stroke:#16a34a
```
...
| 데모 패리티 감사 | ✅ 2026-09-10 | 데모↔실서버 **경로** 패리티 감사: B(데모 과잉) 0건 / C(백엔드-only) 4건 프런트 미사용 정당 예외. `demoClient.DEMO_ROUTES` 테이블화 + `scripts/check-demo-parity.mjs`(백엔드 라우터 introspection ↔ 데모 테이블 대조, `verify.sh` "▶ 데모 패리티 확인" 편입, TC-PARITY-01/02). 새 백엔드 라우트 없음. ADR-0026 부록. 후속: shape 패리티(응답 필드) — `GET /health` 형태 차이 기록 |
| P8 후속 · OKR 구글식 등급 | ✅ 2026-09-14 | 개인 OS P8 후속, 신규 요구사항 **FR-OKR-07** (ADR-0034 채택). 구글 OKR 채점 방식 반영: `key_results.kind`(`committed`/`aspirational`) 컬럼 추가 — 첫 forward-only 마이그레이션 실질 적용(`SCHEMA_VERSION` 1→2, P6 러너 재사용). `backend/src/services/okr.js` 에 `krGrade`·`objectiveGrade` 등급 계산(빨강/노랑/초록 밴드, committed/aspirational 기준 상이) 추가, `GET /api/okr` 응답에 `grade` 필드 포함. 프런트: `okrMath.js` 등급 파생 + `useOkrStore.js` 반영 + `OkrWidgetView.jsx` 등급 칩·`kind` 선택 UI. 데모 어댑터(`demoClient.js`·`demoData.js`) 동기화. 검증: backend 147/147, frontend 107/108(TC-P8-DEMO-05 1건 실패 — **이번 변경과 무관한 기존 결함**, 오늘 날짜 2026-09-14 기준 `demoClient.test.mjs:186` 날짜 의존 시드 계산 버그, git stash 격리로 무관함 확인), `verify.sh` 49/0/0, `check-demo-parity.mjs` 일치 29/예외 4, `check-docs.sh` 11/0/0. supervisor PASS + 비차단 편차 3건(ADR-0034 DESIGN.md §2 등재, 데모 테스트 ID `TC-P8-DEMO-06`, `TC-DB-05/06` user_version 기대값 1→2) 전부 승인·정리 |
| P10 기대정렬 체크인 | ✅ 2026-09-14 | 사용자 고유 자기 점검 방법론(7질문: 뭘하고있지/왜하지/언제까지/목표/전략/구체적으로/상태)을 기능화, 신규 요구사항 **FR-CHECKIN-01~05** (ADR-0035 채택). 전용 테이블 `expectation_checkins` 1개(`schema.sql`, `CREATE TABLE IF NOT EXISTS` — `SCHEMA_VERSION` 미상향, P6/P8 선례) + `backend/src/db.js` prepared stmts + `services/checkins.js`(검증·CRUD) + `routes/checkins.js`(`GET/POST /api/checkins`·`PUT/DELETE /api/checkins/:id`), `project_id` 느슨 FK(`ON DELETE SET NULL`, ADR-0012 관례). 프런트: `useCheckinStore.js`(CRUD + 낙관적 갱신) · `widgets/views/CheckinWidgetView.jsx`(타임라인 + 접이식 7질문 폼) · `widgetMeta`·`registry`·`defaultLayout`·`topics`·`TopicIcons.jsx` 배선(configSchema 포함). 데모 어댑터(`demoClient.js`·`demoData.js`) 동기화. 검증: backend 157/157, frontend 114/114, `verify.sh` 49/0/0, `check-demo-parity.mjs` 일치 33/예외 4, `check-docs.sh` 11/0/0. supervisor PASS — 2라운드 리뷰 완주(1차 지적 3건: configSchema 미배선/블로커·빈 상태 ErrorBanner 누락/경미·데모 길이검증 없음/권장 — 전부 2차에서 해결 확인). 비차단 후속 2건 남김(데모 클라이언트 에러 문구 백엔드와 불일치 — 도달 불가 경로, `TC-P10-DEMO-01` 길이초과 400 케이스 테스트 보강). 로컬 GUI 수동 검증 백로그 |
| P11 지식 축적 추세·역량 지도 | ✅ 2026-09-15 | 신규 요구사항 **FR-KNOW-01~04** (ADR-0036 채택). 읽기 전용 파생 뷰 — 새 테이블 없음, 체크인(주 단위)·OKR(월 단위) 비대칭 그대로 유지, `kr_snapshots` 신규 적재 없음. 백엔드: `backend/src/db.js`(`getCheckinCountsByWeek`·`getTagCountsInRange`·`getKrTrendFrom`) + `services/knowledgeTrend.js`·`routes/knowledgeTrend.js`(신규) + `api.js` 마운트, `GET /api/knowledge-trend`. 프런트: `store/useKnowledgeStore.js`(신규) · `widgets/views/KnowledgeWidgetView.jsx`(신규) · `components/{LineChart.jsx,linePath.js}` 하위 호환 확장(다중 시리즈) · `widgets/{registry,defaultLayout,widgetMeta,topics}.js`·`components/TopicIcons.jsx` 배선. 데모 어댑터(`demoClient.js`) `knowledgeTrend()`·`DEMO_ROUTES` 동기화. 검증: backend 165/165, frontend 121/121, `verify.sh` 49/0/0, `check-demo-parity.mjs` 일치 34/예외 4, `check-docs.sh` 11/0/0. supervisor PASS 전제 조건부 — 리뷰 1차 지적 1건(DESIGN.md §2 ADR-0036 미등재, 차단)을 오케스트레이터가 직접 보완해 통과. 비차단 후속 2건 남김(`listTrendFrom` 쿼리에 `toMonth` 상한 조건 명시적 추가 권장, `getTagCountsInRange` 정렬 주석 문구 정정). 로컬 GUI 수동 검증 백로그 |
| P12 레퍼런스 자료 요약 절차 추적 | ✅ 2026-09-15 | `my-setup-proj-story` RAW_STORIES #7("어떤 요약 절차를 밟았는지 확인 필요")을 닫는 마지막 정합성 감사 공백 해소(ADR-0037 채택, §5-3 공백 1→0). 신규 요구사항 **FR-REF-01~06**. 부모-자식 2테이블 `reference_materials`+`reference_summary_steps`(`schema.sql` CREATE IF NOT EXISTS, `SCHEMA_VERSION` 미상향, ADR-0030·0035 선례) — 요약 단계는 추가·삭제만(PUT 없음, 삭제 후 재번호 안 함). `backend/src/services/references.js`·`routes/references.js`(신규) — `GET/POST /api/references`(category·status·project_id·limit 필터), `PUT/DELETE /api/references/:id`, `POST/DELETE .../steps`. `project_id` 느슨 FK(`ON DELETE SET NULL`, ADR-0012 관례). 프런트: `useReferenceStore.js`(신규) · `widgets/views/ReferenceWidgetView.jsx`(신규) · `widgetMeta`·`registry`·`defaultLayout`·`topics`·`TopicIcons.jsx` 배선. 데모 어댑터(`demoClient.js`·`demoData.js`) 동기화. 검증: backend `npm test` 177/177, frontend `npm test` 129/129, `verify.sh` 49/0/0(문서 정합 11/0/0 포함), `check-demo-parity.mjs` 일치 40/예외 4. 비차단 후속 3건: (1) 위젯에 상태 변경 UI 없음(목록·필터만, todo→reading 등 전환은 백로그) (2) `widgetMeta.configSchema` 의 statusFilter 는 초깃값만 반영되고 실행 중 변경 미배선 (3) `작업로그.md` 요약이 이전(P11) 브랜치 이월 내용으로 남아있던 건 — 이번 커밋에서 `git checkout` 으로 되돌리고 Stop 훅이 재작성하도록 처리. 로컬 GUI 수동 검증 백로그 |
| 보안 보완 S1·S2 | ✅ 2026-09-15 | REVERSE_PLAN §4-5 S1(백엔드 LAN 노출)·S2(CORS `Origin: null` 전 빌드 허용) 해소, 신규 **NFR-SEC-08** + **NFR-SEC-06** 개정. `backend/src/server.js` `app.listen(PORT, '127.0.0.1', …)` 로 루프백 고정(ADR-0004 각주). `backend/src/middleware/cors.js` `Origin: null` 허용을 `APP_ENV=packaged` 요청 시점 분기로 한정, 미설정=거부가 기본값(ADR-0016 2항 각주 — 주입 주체는 2~4항 확정 시 결정). `.env.example`·`ENV_REFERENCE.md` 에 `APP_ENV` 등재. 테스트: `middleware.test.js` TC-MW-04 개정 + TC-MW-10 신규, `lifecycle.test.js`·`scripts/smoke.sh` 는 루프백 고정 의도를 일관되게 반영해 `localhost`→`127.0.0.1`. 검증: backend 166/166, `verify.sh` 49/0/0(스모크 포함), `check-docs.sh` 11/0/0. REVERSE_PLAN §4-7 도식·§6-2 남은 일 갱신(9개→7개 약점) |
| 보안 보완 S3·S5 | ✅ 2026-09-16 | REVERSE_PLAN §4-5 S3(태그 비침범이 코드 가드뿐)·S5(시크릿 스캔이 `.gitignore` 한 겹) 해소 — §4-5 7건→5건, 남은 것 S4 뿐. S3: `backend/db/schema.sql` `trg_task_tags_agent_no_override` (`BEFORE INSERT ON task_tags`, `source='agent'`이고 같은 할일에 `source='user'` 행 존재 시 `RAISE(IGNORE)`) — `SCHEMA_VERSION` 미상향(`CREATE TRIGGER IF NOT EXISTS`, ADR-0018 각주 신설). 기존 3중 코드 가드(`add_agent_tags`·`classify.py`·`get_untagged_tasks`)를 대체하지 않고 4번째 겹으로 이중화(ADR-0029 갱신). S5: `scripts/check-secrets.sh`(8패턴: anthropic·google-api·google-oauth-secret·slack-token·slack-webhook·fernet-key·private-key·filled-env) + `.githooks/pre-commit`(스테이징 파일 스캔, `SECRET_SCAN_SKIP=1` 우회) + `setup.sh` 가 `git config core.hooksPath .githooks` 자동 설정(멱등) + `verify.sh`에 "▶ 시크릿 스캔"(`--staged`, docs/ 거짓양성 0 확인 TC-SEC-03) 신설 + `.gitignore` 에 `*.pem`/`*.key`/`credentials.json` 등 추가. 신규 요구사항 없음(NFR-SEC-01 갱신). 테스트: `db.test.js` TC-DB-08a~d(신규 4), `agent/tests/test_db.py` TC-AGENT-46, `agent/tests/test_classify.py` TC-AGENT-47(경합 시나리오). 검증: backend `npm test` 182/182(기존 178+신규4), agent `pytest` 84 passed/2 skipped, `verify.sh` 54/0/0, `check-docs.sh` 11/0/0, `check-secrets.sh --self-test` 8패턴 탐지+정상파일 0건 exit 0. supervisor 2회차(1차 CHANGES_NEEDED → 수정 → PASS). 비차단 후속 1건: `check-secrets.sh` 의 `--files` 모드가 개행으로 끝나지 않는 파일의 마지막 줄을 못 읽는 엣지케이스(커밋 차단 경로인 `--staged`/훅에는 영향 없음) |
| 보안 보완 S4 (OAuth 키 회전 런북) | ✅ 2026-09-16 | REVERSE_PLAN §4-5 구 S4(OAuth 토큰 암호화 키 회전 절차 없음) 해소 — §4-5 **S1~S5 전부 해소**, 남은 건 D1~D4뿐. 코드 변경 없음 — `docs/setup/ENV_REFERENCE.md` `TOKEN_ENCRYPTION_KEY` 절에 회전·유출 대응 런북 추가(정기 주기 없음, 유출 트리거 시 회전 + Google 계정 OAuth 권한 취소 절차 포함). `ADR-0024`·`RISKS.md` R-14(🔴→🟢) 갱신, §4-7 도식 S4 노드 삭제. 검증: `bash scripts/check-docs.sh` 11/0/0(문서만 변경) |
| ADR-0016 2~4항 데스크톱 프로세스 토폴로지 | ✅ 2026-09-16 | PERSONAL_OS §4-3 앱 통합 마지막 조각. Electron `main.js` 가 백엔드를 `child_process.fork` 로 감독(기동 → `/api/health` 폴링 + 10초 타임아웃 → 실패 시 1s/2s/4s 백오프 최대 3회 재기동 → 포트 3000~3010 순차 탐색 → 앱 종료 시 자식 정리, stdout/stderr 파일 로그). 신규 `frontend/src/main/{backoff,portFinder,healthCheck,backendSupervisor,backendLog,apiBaseUrl}.js` + 에러 화면 `error.html`/`error.js`. `preload.js`·`index.html`·`vite.config.js`·`AppShell.jsx`·`api/client.js`·`backend/src/server.js` 배선. 신규 env `APP_SUPERVISE_BACKEND`(`ENV_REFERENCE.md` 등재), `NFR-DEPLOY-04` 로그 경로 보강, `RUNTIME_VIEW.md` RT-2~4 반영, `ADR-0016` 2~4항 채택 기록 추가, `TEST_PLAN.md` §3.6b + `TC-TOPO-01~18` 등재. `frontend/test/{backoff,portFinder,healthCheck,backendSupervisor,apiBaseUrl}.test.mjs` 신규. supervisor 최종 PASS — **4라운드 리뷰**를 거치며 동시성 버그(중복 기동 경합, generation 가드에서 자식 프로세스 방치, 종료 시퀀스와 재기동 타이머 경합) 전부 해소 확인. 검증: backend 178/178, frontend 147/147, `verify.sh` 49/0/0(문서 정합 11/0/0 포함). 비차단 후속 2건: (1) `backendSupervisor.js` 방어 코드 블록 주석이 실제 도달 조건과 미세하게 불일치(무해) (2) `before-quit` 진행 중 에러창에서 "재시도" 클릭 시 `stopped` 플래그가 풀리는 극히 좁은 이론적 경합 창(이번 범위 밖, 재현 조건 사실상 없음) |
| REST 오류 응답 계약 (ADR-0017) | ✅ 2026-09-16 | REVERSE_PLAN §4-5 **D2 해소**(2건→1건, 남은 건 D1·D3·D4). ADR-0017 채택 — 모든 백엔드 4xx/5xx 응답을 RFC 9457(Problem Details) 스타일 `{type, title, status, detail, errors?, request_id}` + `Content-Type: application/problem+json` + `X-Request-Id` 로 통일, 기존 단일 봉투 `{ error: "한국어" }` 대체. 신규 `backend/src/problem.js`(`PROBLEM_TYPES` 6종 + `sendProblem`/`sendServiceError` 단일 원천, `NotFoundError`→404 · `ConflictError`→409 · `UpstreamUnavailableError`→503 · `ValidationError`→400 분기 전부 배선) + `backend/src/middleware/requestId.js`(신규). `backend/src/errors.js`(`ConflictError`/`UpstreamUnavailableError` 추가). 라우트 15개 전수 전환 + 7개 파일의 중복 `handleError` 제거. `frontend/src/api/{client,demoClient}.js` 파싱 갱신(스토어 `error` 문자열 동작 무회귀). 신규 `backend/test/errorContract.test.js`(TC-ERR-01~07)·`frontend/test/client.test.mjs`(TC-ERR-08). 검증: backend `npm test` 190/190, frontend `npm test` 151/151, `bash verify.sh` 54/54(문서 정합 11/11 포함), `check-demo-parity.mjs` 무영향. supervisor PASS — 비차단 후속 3건(API_REFERENCE.md 404 설명 문구, `sendServiceError` 의 conflict/upstream 미분기, TEST_PLAN.md 수치 오기) 전부 이번 커밋에서 반영, 남은 후속 없음 |
| 다음 | ⏳ | **앱 통합(PERSONAL_OS §4-3) 완료** — ADR-0016 1~4항 전부 채택·구현(통합 실행 스크립트 ✅ 2026-09-09, 데모↔실서버 경로 패리티 ✅, 루프백 바인딩 ✅ 2026-09-15, 패키징 앱 백엔드 감독·재기동·포트 폴백 ✅ 2026-09-16). §4-5 보안 보완 **S1~S5 전부 해소**, REST 오류 계약 **D2 해소**(2026-09-16), 남은 건 D1·D3·D4 기술 부채. 남은 것: P7·P8·P9·P10·P12 로컬 수동 검증 백로그(TC-ACT-M/TC-AGENT-M, TC-OKR-M/TC-PLAN-M, TC-P9-M, TC-DEV-M1~3) + §4-5 D1·D3·D4 기술 부채 |
...
**마지막 업데이트:** 2026-09-14 (개인 OS P10 완료 — 기대정렬 체크인)  
**다음 업데이트:** 2026-09-15 (매주 월요일)
