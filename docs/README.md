...
| [SYSTEM_ARCHIVE.md](SYSTEM_ARCHIVE.md) | 시스템 아카이브 — 다음 세션이 이 문서 하나만 읽고 전체 그림(정체성·목적·기능·아키텍처·ADR 하이라이트·코드 구조·진행 이력·남은 일·개발 방식)을 복원하도록 만든 종합 요약. 정본 문서를 대체하지 않는 안내판 |
| [STUDY_GUIDE.md](STUDY_GUIDE.md) | 아키텍처를 더 공부하려면 — 부족분 진단 + 도서·표준 목록 (C4/arc42, SA in Practice, Release It!, DDIA, RFC 9457 …) |
| [REVERSE_PLAN.md](REVERSE_PLAN.md) | 역 계획서 — 완성된 시스템(P0~P10)을 **원초적 고민 → 의사결정 → 문제해결방법 → 설계 → 서비스** 순으로 되짚는 회고. §1-0 목적(진척 가시성이 아니라 지식의 시각화·역량 방향 가시화), §5-3 기능-목적 정합성 감사, §6-0 기대정렬로 보는 프로젝트. 발표·회고·다음 방향 논의 기반 |
