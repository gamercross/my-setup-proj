...
|---|---|---|---|
| [API_REFERENCE.md](API_REFERENCE.md) | 백엔드 REST 엔드포인트별 요청/응답 예시, 파라미터, 검증 규칙, 상태코드, 부작용, curl | 라우트·프론트 API 배선 시 | "현재 구현과의 차이" 표. 오류 형태는 RFC 9457 스타일 봉투 — [ADR-0017](../architecture/adr/ADR-0017-rest-error-contract.md)(채택) |
...
| [UI_STYLE.md](UI_STYLE.md) | UI/UX 참조 틀 — 레이아웃 골격, 컴포넌트 패턴 6종(P1~P6), 우리 기능→틀 매핑, 안 가져오는 것, 디자인 토큰 정합. 참조 대상은 릴스 "Claude 워크스페이스 대시보드" | 새 화면·컴포넌트 시각 방향 잡을 때 | **계약이 아니라 방향** — 충돌 시 코드·UI_SPEC 우선. 강조색 앰버→보라 전환은 US-1(미결) |
| [DATA_DICTIONARY.md](DATA_DICTIONARY.md) | 테이블·컬럼 단위 의미·타입·제약·예시, 관계 요약 | 쿼리 작성, 필드명 확인 | **단일 원천은 `backend/db/schema.sql`** — 이 문서는 설명. §13 `widget_instances` 는 제안(1차 localStorage) |
