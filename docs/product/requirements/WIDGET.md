...
>
> 상태: 전부 ⏳ (제안). 착수 전 [DASHBOARD_OS.md](../vision/DASHBOARD_OS.md) §8 의 DO-1~6 결정 필요.
...
**우선순위** P1 · **목표 주차** W4~5 (Phase C5) · **상태** ⏳
...
- **AC-4** When 창 너비가 좁아짐, Then 그리드 열 수가 줄고 위젯이 세로로 재배치된다(반응형).
- **엣지** 위젯이 뷰포트 밖으로 나가면 저장 시 보이는 영역으로 clamp.

## FR-WIDGET-02 — 위젯 추가·제거 (생명주기)

**우선순위** P1 · **목표 주차** W4~5 · **상태** ⏳

- **AC-1** When "+ 위젯" 클릭, Then 레지스트리에 등록된 위젯 타입 목록(이름·아이콘·설명)이 뜬다.
- **AC-2** When 타입 선택, Then 기본 크기의 위젯 인스턴스가 빈 자리에 추가되고 즉시 데이터를 로드한다.
- **AC-3** When 위젯 타이틀바의 ✕ 클릭, Then 확인 후 제거되고 레이아웃이 저장된다. 도메인 데이터는 삭제하지 않는다.
- **AC-4** When 타이틀바의 ─ 클릭, Then 위젯이 최소화(타이틀바만)되고, 다시 클릭하면 복원된다.
- **AC-5** (DO-2 결정에 따라) 같은 타입을 여러 개 추가할 수 있다 / 없다.
...
- **AC-3** Given 저장된 레이아웃이 없음(첫 실행), Then 기본 레이아웃(할일·프로젝트·브리핑 위젯)이 적용된다.
...
- **AC-1** `frontend/src/widgets/registry.js` 에 `{ type, name, icon, defaultSize, minSize, maxSize, view, configSchema, useData }` 항목을 추가하면 피커·셸이 자동 인식한다.
