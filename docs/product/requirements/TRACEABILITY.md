...
| FR-OKR-01~05 | — | [requirements/OKR.md](OKR.md), [ADR-0030](../architecture/adr/ADR-0030-okr-data-model.md), [ADR-0018](../architecture/adr/ADR-0018-schema-migration-strategy.md), DATA_DICTIONARY `objectives`/`key_results`/`kr_snapshots`, API_REFERENCE `/api/okr*`·`/api/planner/weekly` | 개인 OS P8 | TC-OKR-01~ (P8 에서 작성) | `backend/db/schema.sql`, `backend/db/index.js` (마이그레이션), `backend/src/db.js`, `backend/src/services/okr.js`(신규), `backend/src/routes/okr.js`(신규), `backend/src/routes/planner.js`(신규), `agent/`(월별 스냅샷 — 선택), `frontend/src/api/demoClient.js`·`demoData.js`(목) | ⏳ P8 다음 |
| FR-OKR-06 | — | [requirements/OKR.md](OKR.md), [ADR-0032](../architecture/adr/ADR-0032-sidebar-shell-per-topic-layouts.md), [ADR-0027](../architecture/adr/ADR-0027-light-theme-default.md), UI_SPEC (OKR·주간 플래너 위젯), PERSONAL_OS §4 | 개인 OS P8 | TC-OKR-M (수동), TC-OKR-20~ (자동, P8 에서 작성) | `frontend/src/store/useOkrStore.js`(신규), `frontend/src/widgets/views/OkrWidgetView.jsx`(신규), `frontend/src/widgets/views/PlannerWidgetView.jsx`(신규), `frontend/src/widgets/{registry,defaultLayout,widgetMeta}.js`, `frontend/src/components/{StatTile,DotProgress,Chip}.jsx`(P4 재사용) | ⏳ P8 다음 |
...
| [ADR-0030](../architecture/adr/ADR-0030-okr-data-model.md) OKR 데이터 모델 | FR-OKR-01~06 | ✅ 채택 (2026-09-08, PO-5/6) — 1급 엔티티 3테이블, 주간 플래너 순수 집계. 구현은 개인 OS P8. 요구사항 [OKR.md](OKR.md) |
