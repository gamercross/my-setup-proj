...
| FR-TASK-08 | 할 일에 자유 문자열 태그(다중)를 달 수 있고, 태그 없는 할 일은 에이전트가 일일 브리핑 배치에서 자동 분류한다 | P1 | 개인 OS P6 | ✅ (2026-09-08, ADR-0018·ADR-0029) |
| FR-TASK-09 | 할 일 위젯을 리스트/보드(우선순위 칸반)로 전환하고, 어느 뷰에서 완료해도 같은 상태다 | P5 | 개인 OS P5 | ✅ (2026-09-08, ADR-0028) |
...
| FR-AGENT-03 | 생성된 브리핑을 Notion 페이지로 저장한다 | P1 | W7 | ✅ (D3) |
| FR-AGENT-04 | 브리핑을 로컬 DB에 캐시하고 UI에서 조회할 수 있다 (빈 결과 200+`{brief:null}` — ADR-0025) | P1 | W7 | ✅ (D3) |
| FR-AGENT-05 | 매일 아침(기본 07:30) 정해진 시각에 브리핑이 자동 생성된다 (cron/launchd) | P0 | W7 | ✅ (D3) |
| FR-AGENT-06 | Claude 호출 실패 시 사용자에게 원인을 알리고 앱은 계속 동작한다 | P0 | W7 | 🚧 |
| FR-AGENT-07 | 스케줄 제안: 바쁜 시간대를 분석해 회의 가능 시간을 제안한다 | P2 | W11 | ⏳ |
| FR-AGENT-08 | 에이전트 활동 위젯: 최근 실행 이력·연결 상태·다음 예약을 보여주고 "지금 실행"을 요청한다 (파일 플래그 트리거 — ADR-0011/0013) | P1 | W8 | ✅ (P7, 2026-09-08) |
| FR-AGENT-09 | 대시보드 기반 작업 큐: 요청을 입력하면 에이전트가 처리해 결과를 돌려준다 (Daily Brief 일반화, [ADR-0013](../architecture/adr/ADR-0013-dashboard-agent-queue.md)) | P2 | W11+ | ⏳ |
...
| FR-SYNC-03 | 외부 API 동기화 결과를 `sync_logs` 에 기록한다 (성공/실패/오류메시지) | P1 | W6 | ✅ (D2-a·D2-b) |

## 7. 사용자 / 인증 (AUTH)

| ID | 요구사항 | 우선순위 | 목표 주차 | 상태 |
|---|---|:---:|:---:|:---:|
| FR-AUTH-01 | Google OAuth 2.0 으로 로그인하고 refresh token 을 로컬에 암호화 저장한다 | P1 | W6 | ✅ (D2-b, 2026-09-07) |
...
| FR-UI-06 | 진행 현황 위젯에서 파일 트리를 훑고 `.md` 문서 본문을 안전하게 렌더한다 | P2 | 개인 OS P9 | ✅ 구현 (2026-09-08), E2E 대기 |
...
| ID | 요구사항 | 우선순위 | 목표 주차 | 상태 |
|---|---|:---:|:---:|:---:|
| FR-WIDGET-01 | 위젯을 그리드 위에서 이동·리사이즈할 수 있다 (편집 모드) | P1 | W4~5 (C5) | ✅ (C5) |
| FR-WIDGET-02 | 위젯을 피커로 추가하고 ✕/최소화로 제거·접을 수 있다 | P1 | W4~5 (C5) | ✅ (C5) |
| FR-WIDGET-03 | 위젯 클릭 시 최상단으로 오고 포커스 표시된다 (z-order) | P2 | W5 (C5) | ✅ (C5) |
| FR-WIDGET-04 | 레이아웃(위치·크기·z·최소화·config)이 저장·복원된다 | P1 | W4~5 (C5) | ✅ (C5) |
| FR-WIDGET-05 | 위젯마다 테마(색·강조·모서리·밀도·타이틀바)를 따로 지정한다 | P1 | W5~6 (C6) | ✅ (C6) |
