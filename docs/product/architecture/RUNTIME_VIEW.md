...
| # | 질문 | 선택지 | 결정 시점 |
|---|---|---|---|
| RT-1 | dev 에서 백엔드를 `frontend` 명령이 같이 띄우나? | (a) `concurrently` 로 3개 동시 (b) 계속 분리 | 지금 (C 단계 전) |
...
**잠정 권고:** RT-1 = (a) `concurrently` (실수 방지), RT-2 = (a) main 이 fork + 헬스체크 후 창 표시, RT-3 = (a) 재기동 3회 후 배너. Week 5 에 ADR 로 확정.
