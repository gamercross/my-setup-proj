...
- 상태: 전체 큐 **제안** / P7 "지금 실행" 트리거 **채택** (2026-09-08) — 전체 작업 큐는 핵심 기능 완성 후(Week 10+) 재검토.
- **부분 채택 (2026-09-08, PO-9):** 개인 OS **P7**(에이전트 활동 위젯)의 "지금 실행" 버튼은
  **전용 트리거 디렉터리 + 파일 플래그** 방식으로 확정 — 백엔드가 `agent/.triggers/run-now` 플래그 파일을 쓰고
  (`POST /api/agent/run-now`, 멱등), launchd `WatchPaths`(`scripts/install-runnow-launchd.sh`)가 그 디렉터리
  변경을 감지해 `agent/trigger.py` → `sync.sync_all()` 을 1회 실행 후 플래그를 삭제한다. `daily-brief-run.sh`
  도 시작 시 밀린 플래그를 소비한다(폴백). 백엔드가 python 프로세스를 직접 spawn 하지 않으므로
  [ADR-0011](ADR-0011-agent-backend-db-access.md) 프로세스 분리 원칙을 유지한다.
  데모(`demoClient.js`)는 즉시 성공 목으로 처리. 전체 `agent_jobs` 큐(임의 프롬프트)는 여전히 범위 밖.
- 관련: FR-AGENT-08(활동 위젯·트리거) / FR-AGENT-09(전체 큐), [VISION.md](../../vision/VISION.md) 향후 확장, [ORCHESTRATION.md](../../../setup/ORCHESTRATION.md), 강의 Week 11
...
## 미결
- 트리거 방식 → **P7 "지금 실행" 은 전용 디렉터리 + 파일 플래그 + launchd WatchPaths 로 확정 (2026-09-08, PO-9, 위 참조).** 전체 큐의 러너 방식(상주 vs 폴링)은 Week 10+ 에서.
