...
| **P10** ✅ | 기대정렬 체크인 (2026-09-14, ADR-0035 채택, FR-CHECKIN-01~05) | `expectation_checkins` 전용 테이블 1개(`SCHEMA_VERSION` 무변경). 7질문 자유 서술 CRUD·목록/필터·위젯. 채점·완성도 점수 없음 — 자기 점검 기록이 목적 | P4·P5 |
| **P11** ✅ | 지식 축적 추세·역량 지도 (2026-09-15, ADR-0036 채택, FR-KNOW-01~04) | 새 테이블 없음, Claude 호출 없음 — 체크인 주간 빈도 + OKR 월평균 달성률 + 태그 분포를 종합하는 읽기 전용 파생 뷰. `GET /api/knowledge-trend`. §5-3 정합성 감사 공백 중 하나를 닫음 | P8·P10 |
| **P12** ✅ | 레퍼런스 자료 요약 절차 추적 (2026-09-15, ADR-0037 채택, FR-REF-01~06) | `reference_materials`+`reference_summary_steps` 2테이블(부모-자식, `SCHEMA_VERSION` 무변경). 요약 단계는 추가·삭제만(수정 없음). §5-3 정합성 감사 **마지막 공백을 닫음(공백 1→0)** | P4·P5 |
