...
export function createDataset() {
  ...
  const tasks = [
    ...
    // dueDays 는 0 이상만 쓴다 — 음수(예: -1 "어제")는 오늘이 월요일일 때 ISO 주
    // 경계를 넘어 지난주 버킷으로 새어 들어가 TC-P8-DEMO-05 의 lastWeek.total 을
    // 날짜에 따라 흔든다(재현: 월요일에 실행). "오늘 마감, 완료"로도 의미는 유지된다.
    t(5, '페이지 교체 알고리즘 정리 노트', 'medium', 'done', 0, 2, ['공부']),
